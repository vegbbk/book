...
- (void)viewWillLayoutSubviews {
    ...
    frame.size.height = self.view.frame.size.height - self.tabHeight-30;
...
//     activeTabView.indicatorColor=[UIColor redColor];
...
- (void)defaultSettings {
    ...
    _tabsViewBackgroundColor = [UIColor redColor];
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        _tabWidth = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:CZScreenW/3];
